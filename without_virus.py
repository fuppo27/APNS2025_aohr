...
def pop_info(G, landmark_dtf, pop_length, route_order, end_node):
    # ランドマーク順序に基づいて最短経路を計算
    route = []
     # 各ランドマーク間の経路を計算
    for index in range(pop_length + 1):
        # ランドマークの現在地（source）と次の目的地（target）のノードIDを取得
    ...
    return (node_num, route, route_order, length, time)
...
def crossover(pop_dtf, landmark_dtf, pop_length, landmark_num, crossover_prob):
    ...
    if check_prob <= crossover_prob:
        ...
        for index in duplication_list:
            new_pop_1.append(index)
            new_pop_2.append(index)
            
        for index in parents_dtf.index:
            if index in duplication_list:
                continue

            if len(new_pop_1) >= pop_length:
                new_pop_2.append(index)
            else:
                new_pop_1.append(index)
        new_pop_1 = sorted(new_pop_1)
        new_pop_2 = sorted(new_pop_2)
    ...
    # スタート地点とゴール地点を追加
    new_pop_1.insert(0, 0)
    new_pop_1.append(landmark_num + 1)
    new_pop_2.insert(0, 0)
    new_pop_2.append(landmark_num + 1)

    # 子個体の経路順序を表示
    text.append(f"子1 長さ：{len(new_pop_1)} --> {new_pop_1}\n")
    text.append(f"子2 長さ：{len(new_pop_2)} --> {new_pop_2}\n")
...
if __name__ == "__main__":
    ...
    # ログファイルのファイル名やその他の設定
    experiment = 'alpha/wo_virus'  # 実験の種類
    ...
    landmark_dtf.to_csv(f'work/{city}/{experiment}/{fp}/landmark.csv')
    ...
    for loop in range(generation_num):
        ...
        while len(next_pops) < pop_num - elite_select_num:
            # 交叉
            pops = crossover(tournament_pop_dtf, landmark_dtf, pop_length, landmark_num, crossover_prob)

            next_pops.append(pops[0])
            if len(next_pops) >= pop_num - (elite_select_num * cluster_num):
                break
            next_pops.append(pops[1])
            if len(next_pops) >= pop_num - (elite_select_num * cluster_num):
                break

        next_pop_list = []
        for next_pop in next_pops:
            pop = pop_info(G, landmark_dtf, pop_length, next_pop, end_node)
            next_pop_list.append(pop)

        next_pop_dtf = pd.DataFrame(next_pop_list, columns=['node_num', 'route_node', 'route_order', 'length', 'travel_time'])

        elite_pop_dtf = pop_dtf[pop_dtf['selection_id'] == 1]
        elite_pop_dtf = elite_pop_dtf[['node_num', 'route_node', 'route_order', 'length', 'travel_time']]
